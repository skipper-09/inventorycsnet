...
class ReportApiController extends Controller
{
    ...
    public function getPsbReport(Request $request){
        $user = $request->user();
        $customer = Transaction::with(['customer:id,name,address','branch','Transactionproduct.product:id,name'])->whereHas('Transactiontechnition', function ($query) use ($user) {
            $query->where('user_id', 2); 
        })->get();
    
        return response()->json([
            'status' => 'success',
            'message' => 'Berhasil Mendapatkan Laporan Pemasangan Baru.',
            'data' => $customer,
        ]);
    }
